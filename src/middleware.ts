...
const isProtectedRoute = createRouteMatcher(['/dashboard(.*)']);
const isRestrictedRoute = createRouteMatcher(['/dashboard/event/:path*', '/dashboard/expense/:path*', '/dashboard/users/:path*']);
...
export default clerkMiddleware(async (auth, req: NextRequest) => {
  if (isProtectedRoute(req)) {
    ...
    if (isRestrictedRoute(req)) {
      ...
      // Allow base route
      if (pathname === '/dashboard/event') return;
      if (pathname === '/dashboard/expense') return;
