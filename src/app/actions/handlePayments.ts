...
export async function getCategorizedPayments() {
  ...
  for (const p of payments) {
    ...
    if (year === currentYear && month === currentMonth) {
      if (today <= 7) {
        categorized.upcoming.push(serializePayment(p)); // 1st to 7th
      } else {
        categorized.due.push(serializePayment(p)); // 8th onwards
      }
    } else if (year > currentYear || (year === currentYear && month > currentMonth)) {
      categorized.advance.push(serializePayment(p)); // any future month
    }
  }
...
export async function getPaymentHistory() {
  ...
  return history.map(serializePayment);
}
